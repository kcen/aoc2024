...
var 
  rules: HashSet[(int, int)] = initHashSet[(int, int)]()
...
proc day_05*(): Solution =
  ...
  for line in getInput().splitlines:
    ...
    if is_rule:
      rules.incl((left_rule, right_rule))
    else:
      try:
        var vals = line.split(',').mapIt(parseInt(it))
        let (was_sorted, mid_val) = sortedMid(vals)
        if was_sorted:
          mid_sorted += mid_val
        else:
          mid_fixed += mid_val
  ...
  result = Solution(part_one: $mid_sorted, part_two: $mid_fixed)
